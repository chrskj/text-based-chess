    ...
    def piece_setup(self, pos):
        ...
        for x in range(0, len(pos), 7):
            for y in range(0, len(pos), 7):
                if x == 0 or x == 1:
                    d['Rook'+str(x)+str(y)] = Rook('B', 1, 2)
                    pos[x][y] = (d['Rook'+str(x)+str(y)].letter).lower()
                else:
                    d['Rook'+str(x)+str(y)] = Rook('W', 1, 2)
                    pos[x][y] = d['Rook'+str(x)+str(y)].letter

        for x in range(0, len(pos), 7):
            for y in range(1, len(pos), 5):
                if x == 0 or x == 1:
                    d['Knight'+str(x)+str(y)] = Knight('B', 1, 2)
                    pos[x][y] = (d['Knight'+str(x)+str(y)].letter).lower()
                else:
                    d['Knight'+str(x)+str(y)] = Knight('W', 1, 2)
                    pos[x][y] = d['Knight'+str(x)+str(y)].letter

        for x in range(0, len(pos), 7):
            for y in range(2, len(pos), 3):
                if x == 0 or x == 1:
                    d['Bishop'+str(x)+str(y)] = Bishop('B', 1, 2)
                    pos[x][y] = (d['Bishop'+str(x)+str(y)].letter).lower()
                else:
                    d['Bishop'+str(x)+str(y)] = Bishop('W', 1, 2)
                    pos[x][y] = d['Bishop'+str(x)+str(y)].letter

        for x in range(0, len(pos), 7):
            for y in range(3, len(pos), 7):
                if x == 0 or x == 1:
                    d['Queen'+str(x)+str(y)] = Queen('B', 1, 2)
                    pos[x][y] = (d['Queen'+str(x)+str(y)].letter).lower()
                else:
                    d['Queen'+str(x)+str(y)] = Queen('W', 1, 2)
                    pos[x][y] = d['Queen'+str(x)+str(y)].letter

        for x in range(0, len(pos), 7):
            for y in range(4, len(pos), 7):
                if x == 0 or x == 1:
                    d['King'+str(x)+str(y)] = King('B', 1, 2)
                    pos[x][y] = (d['King'+str(x)+str(y)].letter).lower()
                else:
                    d['King'+str(x)+str(y)] = King('W', 1, 2)
                    pos[x][y] = d['King'+str(x)+str(y)].letter

        for x in range(1, len(pos), 5):
            for y in range(0, len(pos), 1):
                if x == 0 or x == 1:
                    d['Pawn'+str(x)+str(y)] = Pawn('B', 1, 2)
                    pos[x][y] = (d['Pawn'+str(x)+str(y)].letter).lower()
                else:
                    d['Pawn'+str(x)+str(y)] = Pawn('W', 1, 2)
                    pos[x][y] = d['Pawn'+str(x)+str(y)].letter
